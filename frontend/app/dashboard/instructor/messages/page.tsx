        ...
        <div className="w-72 shrink-0 bg-white border-r border-slate-100 flex flex-col">
          <div className="p-4 border-b border-slate-100">
            ...
            <div className="relative">
              ...
              <input type="text" placeholder="Search…" className="w-full h-9 pl-9 pr-3 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1A6EF5]/20" />
            </div>
          </div>
            ...
            {threads.map((t) => (
              <button key={t.id} onClick={() => setActive(t.id)}
                className={`w-full text-left px-4 py-4 border-b border-slate-50 flex items-start gap-3 transition-colors ${
                  active === t.id ? "bg-blue-50" : "hover:bg-slate-50"
                }`}
              >
                ...
                {t.unread > 0 && (
                  <span className="w-4 h-4 bg-[#1A6EF5] rounded-full flex items-center justify-center shrink-0 mt-0.5">
        ...
        <div className="flex-1 flex flex-col bg-gray-50">
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`flex ${msg.self ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-sm px-4 py-3 rounded-2xl text-sm leading-relaxed ${
                  msg.self
                    ? "bg-[#1A6EF5] text-white rounded-br-sm"
                    : "bg-white border border-slate-200 text-slate-800 rounded-bl-sm"
                }`}>
                  {msg.text}
                  <p className={`text-[10px] mt-1 ${msg.self ? "text-blue-200" : "text-slate-400"}`}>{msg.time}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="bg-white border-t border-slate-100 px-6 py-4 flex items-center gap-3">
            <input type="text" value={draft} onChange={(e) => setDraft(e.target.value)}
              ...
              className="flex-1 h-10 border border-slate-200 rounded-xl px-4 text-sm focus:outline-none focus:ring-2 focus:ring-[#1A6EF5]/20" />
            <button onClick={() => setDraft("")}
              className="w-10 h-10 bg-[#1A6EF5] hover:bg-blue-600 text-white rounded-xl flex items-center justify-center transition-colors">
